...
export const StateContext = ({ children }) =>
{
  ...
  const getProducts = () =>
  {
    axios.get(`${process.env.NEXT_PUBLIC_SERVER}/products`, { timeout: 5000 }).then(res => setPorducts(res.data));
  };
  useEffect(() =>
  {
    getProducts();
  ...
  const checkUser = () =>
  {
    ...
    if (token)
    {
      axios.post(`${process.env.NEXT_PUBLIC_SERVER}/check-user`, { 'token': token })
